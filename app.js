...
const App = {
  ...
  async init() {
    ...
    if (SupabaseService.ready) {
      ...
      if (error) this.showAuthMessage(error.message || 'Không đọc được phiên đăng nhập.', true);
      await handleUserSession(session?.user || null);

      SupabaseService.client.auth.onAuthStateChange(async (_event, sessionNow) => {
        await handleUserSession(sessionNow?.user || null);
      });
    } else {
      this.showAuthMessage('Supabase chưa được khởi tạo đúng trong supabase.js.', true);
    }
  ...
  async saveCloudNow() {
    ...
    try {
      this.setSyncStatus('Đang lưu Supabase...');
      await SupabaseService.saveJournal(this.state.user.id, this.data);
      this.setSyncStatus('Đã lưu lên Supabase');
    } catch (error) {
      console.error(error);
      this.setSyncStatus('Lưu thất bại');
      alert('Không lưu được lên Supabase: ' + (error.message || error));
    } finally {
