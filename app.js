...
function loadState(){
  try {
    ...
    if (saved) {
      ...
      merged.market = { ...deepClone(defaultState.market), ...(saved.market || {}) };
      merged.mindset = { ...deepClone(defaultState.mindset), ...(saved.mindset || {}) };
      merged.review = { ...deepClone(defaultState.review), ...(saved.review || {}) };
      return merged;
    }
  } catch (e) {}
...
async function handleLogin() {
  ...
  try {
    ...
    console.error(error);
    statusEl.textContent = 'Lỗi đăng nhập: ' + error.message;
...
function renderDashboardTab(){
  ...
  const avgQuality = appState.trades.length ? round2(appState.trades.reduce((a,t)=>a+t.quality.total,0) / appState.trades.length) : 0;
...
function renderPositionSizing(){
  const first = appState.trades[0] || { entry: 100, stop: 95 };
